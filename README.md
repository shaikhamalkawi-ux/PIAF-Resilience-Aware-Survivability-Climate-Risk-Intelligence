...
This repository contains the analysis inputs, code, generated numerical tables, and publication-figure workflow for the major revision of:
...
- `scripts/download_nasa_power.py` - raw NASA POWER acquisition, daily-file, provenance, seasonality-diagnostic, and thermal-redundancy workflow.
- `notebooks/ECMX_D_26_01378_NASA_POWER_Acquisition.ipynb` - Google Colab runner that stores raw and processed NASA POWER evidence directly in the project Drive folder.
...
A successful run prints the analysis summary and reports the generated tables and figures.

## Re-acquire the NASA POWER source data

The source-acquisition script retrieves the same ten fixed coordinates and the same five NASA POWER daily variables used by the revision:

```bash
pip install -r requirements.txt
python scripts/download_nasa_power.py --output NASA_POWER_ECMX_R1 --overwrite
```

The acquisition workflow stores the exact raw JSON response for each site, a clean daily CSV, exact query URLs, SHA-256 hashes, the site summary, a GHI seasonality diagnostic, and a T2M/T2M_MAX redundancy audit. The raw daily GHI coefficient of variation is retained only as a transparent historical descriptor; the separate diagnostic reports interannual and month-conditioned variability so seasonal structure is not silently interpreted as pure short-term volatility.
...
For the journal resubmission, the manuscript should cite the exact immutable Git commit associated with the submitted reproducibility files. A DOI can additionally be added if the final release is archived through Zenodo or another DOI-minting repository.
