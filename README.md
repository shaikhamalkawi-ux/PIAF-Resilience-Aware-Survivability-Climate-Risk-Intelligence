...
outputs/
  ...
  sha256_manifest.csv
  tables/*.csv                       version-controlled numerical outputs
```

The figures are generated by `scripts/run_analysis.py` and are intentionally reproducible rather than hand-edited.
...
- The central scenario places TOPCon first at all ten benchmark points, but the declared technology envelopes overlap. The repository therefore does **not** claim guaranteed universal dominance of one PV family.
